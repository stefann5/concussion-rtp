...
- Pri svakom dnevnom unosu (event insert + `fireAllRules()` u `ProtocolService`) okida se trostepeni lanac pravila unaprednog ulančavanja (FC — Forward Chaining): Nivo 1 izvodi setback signale (`ExertionIntoleranceFlag` — kroz CEP accumulate pattern u 48h prozoru — i/ili `RegressTrigger` — kroz CEP temporal join nakon prelaska koraka), Nivo 2 klasifikator izvodi `ProgressionStatusFact`, Nivo 3 akcija izvodi `StepRecommendation` + `ProtocolLockEvent` + opcionalni `modify` athlete fact-a. (`MoreThanMildExacerbation` se takođe izvodi na svaki bump iznad praga, ali kao informativna činjenica na dashbord-u — ne učestvuje u odluci o HOLD/REGRESS.)
...
4. **Nivo 2 (FC)**: Pravilo `"Determine status STOP_AND_RETRY at steps 1-3"` iz agenda grupe `phase-decision` ([ProgressionDecision.drl](kjar/src/main/resources/rules/fc/ProgressionDecision.drl)) gleda `Athlete(currentStep ∈ [1,3])` + disjunkciju `(ExertionIntoleranceFlag or RegressTrigger)`. `ExertionIntoleranceFlag` je upravo ubačen u Nivou 1 → klasifikator se okida i ubacuje `ProgressionStatusFact(athleteId="marko", status=STOP_TODAY_RETRY_TOMORROW)`. Marko je na Koraku 3, pa Amsterdam 2022 logika za Korake 1–3 nalaže da ne vraća unazad — zaustavlja vežbu i pokušava **isti** Korak 3 sledećeg dana. (Pojedinačni `MoreThanMildExacerbation` iz prvog bumpa je takođe u radnoj memoriji ali se ne koristi kao okidač odluke — informativni je.)
...
Klasifikatori prepoznaju setback samo na osnovu **vremenski potvrđenih signala** — `ExertionIntoleranceFlag` (2+ above-threshold događaja u 48h prozoru) ili `RegressTrigger` (simptom iznad baseline-a u prozoru nakon prelaska koraka). Pojedinačni `MoreThanMildExacerbation` se ubacuje na svaki bump iznad praga i prikazuje na dashbord-u kao informativna oznaka, ali **sam za sebe ne pokreće HOLD/REGRESS** — to bi bilo previše osetljivo (jedna teža vežba ne mora značiti da sportista nije spreman).

- **Setback na Koracima 1–3**: ako je sportistin trenutni korak između 1 i 3, **i** postoji `ExertionIntoleranceFlag` ili `RegressTrigger` za njega, klasifikator (`"Determine status STOP_AND_RETRY at steps 1-3"`) ubacuje `ProgressionStatusFact(STOP_TODAY_RETRY_TOMORROW)`. Akciono pravilo (`"Recommend HOLD on current step (steps 1-3)"`, sa `salience -10` da se okine posle klasifikatora u istom ciklusu) ubacuje `StepRecommendation(action=STOP_AND_RETRY, currentStep=cur, recommendedStep=cur, retryAfterHours=24)` i `ProtocolLockEvent(lockUntilHours=24, reason="STOP_AND_RETRY_SAME_STEP")`. Athlete fakat se **ne menja**.
...
Ova četiri pravila iz iste agenda grupe `phase-decision` (sa `auto-focus true`) čine **drugi i treći nivo trostepenog FC lanca**: Nivo 1 izvodi setback signale (`ExertionIntoleranceFlag` kroz CEP accumulate, `RegressTrigger` kroz CEP temporal join — `MoreThanMildExacerbation` se takođe izvodi ali kao informativna činjenica koja ne učestvuje u odluci) → Nivo 2 klasifikator izvodi `ProgressionStatusFact` → Nivo 3 akcija izvodi `StepRecommendation` + `ProtocolLockEvent` + opcionalni `modify` na athlete fact-u.
